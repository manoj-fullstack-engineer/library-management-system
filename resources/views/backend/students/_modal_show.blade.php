...
<div>
    ...
    <div class="tab-content" id="studentTabContent">
            ...
            <div class="row">
                <div class="col-md-3 text-center">
                    <img src="{{ $student->photo ? asset('storage/' . $student->photo) : asset('images/default-user.png') }}"
                        class="img-thumbnail rounded-circle mb-2" alt="Student Photo" width="120">
                    <h6 class="fw-semibold">{{ $student->first_name }} {{ $student->middle_name }} {{ $student->last_name }}</h6>
                ...
                <div class="col-md-9">
                    <table class="table table-sm">
                        <tbody>
                            <tr><th>Registration No.</th><td>{{ $student->registration_number }}</td></tr>
        ...
        <div class="tab-pane fade" id="academic" role="tabpanel">
            <table class="table table-bordered table-sm">
                <tbody>
                    ...
                    <tr><th>Year/Semester</th><td>{{ $student->year_semester }}</td></tr>
                </tbody>
            </table>
        </div>
        ...
        <div class="tab-pane fade" id="library" role="tabpanel">
            <table class="table table-bordered table-sm">
                <tbody>
                    <tr><th>Library Card No.</th><td>{{ $student->student_library_id }}</td></tr>
                    <tr><th>Membership Status</th><td>{{ $student->membership_status }}</td></tr>
                    <tr><th>Total Books Issued</th><td>{{ $student->total_books_issued ?? 0 }}</td></tr>
                    <tr><th>Max Book Limit</th><td>{{ $student->max_book_limit }}</td></tr>
                    <tr><th>Fine Amount</th><td>₹{{ number_format($student->fine_amount ?? 0, 2) }}</td></tr>
                    <tr><th>Blacklist Status</th><td>{{ $student->blacklist_status }}</td></tr>
                    <tr><th>Last Login</th><td>{{ $student->last_login }}</td></tr>
